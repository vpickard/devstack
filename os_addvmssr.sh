...
port3_id=$(neutron port-create multinet1 --name sriov_port3 --binding:vnic_type direct --device-owner nova-compute | grep "\ id\ " | awk '{ print $4 }')
port4_id=$(neutron port-create multinet1 --name sriov_port4 --binding:vnic_type direct --device-owner nova-compute | grep "\ id\ " | awk '{ print $4 }')
...
#1st sriov vm on .19 using port1
...
#2nd sriov vm on .30 using port2
...
#3rd sriov vm on .19 using port3
...
#4th sriov vm on .30 using port4
nova boot --config-drive=true --poll --flavor m1.small --image centos7 --nic port-id=$port4_id --availability-zone=nova:nfvsdn-20.oot.lab.eng.rdu2.redhat.com --key-name admin_key --user-data /opt/tools/centos.yaml sriov-vm4
...
nova get-vnc-console sriov-vm4 novnc
